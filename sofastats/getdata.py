...
class DataDets:
    ...
    def set_proj_dic(self, proj_dic, dic2restore=None):
        ...
        try:
            ...
            add_checks = False
            self.set_dbe(
                dbe, db, tbl, add_checks, con_dets, default_dbs, default_tbls)
    ...
    def set_db(self, db, tbl=None):
        ...
        :param str tbl: if None will use the default as per the proj settings.
        """
...
def get_dbe_szrs(dlg_proj, scroll, lblfont, *, read_only=False):
    ""
    dbe_szrs = []
    for dbe in mg.DBES:
        dbe_szr = mg.DBE_MODULES[dbe].get_dbe_szr(
            dlg_proj, scroll, lblfont, read_only=read_only)
        dbe_szrs.append(dbe_szr)
    return dbe_szrs
...
def get_db_dets(db_path, db_names):
    if db_path.endswith(('.xls', '.xlsx', '.csv', '.dat', 'tsv')):
        msg = ("Expecting a database file but instead got what"
            f" looks like a spreadsheet or text dataset ('{db_path}')")
        wx.MessageBox(msg)
        raise Exception(msg)
    ...
    return db_name, db_name_key

def gui_connection_info(dlg_proj, default_dbe):
    r"""
    ...
    for dbe in mg.DBES:
        mandatory = (dbe == default_dbe)
        dbe_mod = mg.DBE_MODULES[dbe]
        dets = dbe_mod.gui_connection_info(dlg_proj, mandatory=mandatory)
        default_db, default_tbl, dbe_con_dets = dets
        if default_db:
            default_dbs[dbe] = default_db
        if default_tbl:
            default_tbls[dbe] = default_tbl
        if dbe_con_dets:
            con_dets[dbe] = dbe_con_dets
    return default_dbs, default_tbls, con_dets
...
def is_default_db(dd):
    return (dd.dbe == mg.DBE_SQLITE and dd.db == mg.SOFA_DB)
