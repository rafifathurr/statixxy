...
of the report table. All it does is collect the right information to pass onto
the script. The same configuration could be hand typed into the script by the
user with the same effect.

The script utilises dimtables to actually generate the HTML. Firstly, the node
information is turned into a label tree which is used to create the data cells.

A label tree has nodes for each value under a variable e.g. the dimension might
be country vs agegroup but we still need to know if there is a label node for a
particular value e.g. '30-39' for the given (filtered) data. Plus we need to add
label nodes for totals, row %s etc.
"""
...
class DlgSelectVars(wx.Dialog):
    ...
    def __init__(self, parent, var_choices, selected_idxs):
        wx.Dialog.__init__(self, parent=parent,
            title=_("Select variable(s)"), style=wx.CAPTION|wx.SYSTEM_MENU)
        self.selected_idxs = selected_idxs
        szr = wx.BoxSizer(wx.VERTICAL)
        self.var_selector = wx.ListBox(
            self, -1, choices=var_choices, style=wx.LB_MULTIPLE, size=(-1, 300))
...
class DimTree:
    ...
    ItemConfig object as its PyData (using set_initial_config(). This includes
    on_col_config when col_no_vars_item is added.
    """
    ...
    def on_row_add_under(self, _event):
        """
        ...
        if len(selected_ids) == 1:
            ...
            wx.MessageBox(
                _("Can only add under a single selected row variable."))
            return

    def on_col_add_under(self, _event):
        """
        ...
        if len(selected_ids) == 1:
            ...
            wx.MessageBox(
                _("Can only add under a single selected column variable."))
            return

    def try_adding_under(self, tree, root, dim,
            oth_dim, selected_id, oth_dim_tree, oth_dim_root):
        """
        Try to add var under selected var.
        Only do so if OK e.g. no duplicate text in either dim.
        """
        dd = mg.DATADETS_OBJ
        var_names = list(dd.flds.keys())
        (sorted_choices, 
         sorted_vars) = lib.GuiLib.get_sorted_choice_items(
            ...
            for ancestor in lib.GuiLib.get_tree_ancestors(tree, new_id):
                ...
                if item_conf:  ## ignore root node
                    ...
                    if item_conf.sort_order in (
                            mg.SORT_INCREASING_LBL, mg.SORT_DECREASING_LBL):
    ...
    def on_item_delete(self, dim):
        ...
        measures based on that selected item ready for reuse when adding new
        items.
        """
        if dim == mg.ROWDIM_KEY:
            ...
            raise Exception("Missing appropriate dim for on_item_delete().")
        selected_ids = tree.GetSelections()
        if not selected_ids:
    ...
    def on_row_delete(self, _event):
        self.on_item_delete(dim=mg.ROWDIM_KEY)

    def on_col_delete(self, _event):
    ...
    def config_dim(self, dim):
        ...
        Either with columns variables or without. If without, total doesn't make
        sense.
        """
        debug = False
        if dim == mg.ROWDIM_KEY:
            ...
            raise Exception("Missing appropriate dim for config_dim().")
        ## error 1
        ...
        if empty_tree:
            raise Exception(f"Cannot configure a missing {itemlbl} item")
        # error 2
        selected_ids = tree.GetSelections()
        if not selected_ids:
            wx.MessageBox(
                _("Please select a %s variable and try again") % itemlbl)
            return
        ...
        for selected_id in selected_ids[1:]:
            if lib.GuiLib.item_has_children(
                tree=tree, parent=selected_id) != first_has_children:
        ...
        if no_vars_item in selected_ids:
            ...
            raise Exception(f"Configuring a {itemlbl} but lacking either vars "
                "OR a config item")
        if dim == mg.ROWDIM_KEY:
        ...
        if any_config2get:
            ...
            dlg = DlgConfig(parent, selected_ids, tree, title,
    ...
    def on_row_config(self, _event):
        "Configure row button clicked."
        self.config_dim(dim=mg.ROWDIM_KEY)

    def on_col_config(self, _event):
    ...
    def setup_row_btns(self):
        ...
        """
        has_rows = bool(lib.GuiLib.get_tree_ctrl_children(
...
class DlgConfig(wx.Dialog):
    ...
    def __init__(self, parent, node_ids, tree, title, allow_tot,
    ...
    def on_btn_select_toggle(self, _event):
    ...
    def _set_btn_lbl(self):
        btn_select_lbl = (mg.DESELECT_ALL_LBL if not self.button2showall 
            else f" {mg.SELECT_ALL_LBL} ")  ## select narrower in English at least
    ...
    def on_ok(self, _event):
        ...
        if self.sort_opt_allowed == mg.SORT_NO_OPTS:
            ...
            try:
                ...
                raise Exception("Unexpected sort type")
        ...
        when we add more items, we can default to the same settings if possible.
        Note - deep copy to guarantee staying independent. The other object is
        used to set the ItemPyData for a particular node in the GUI.
        """
