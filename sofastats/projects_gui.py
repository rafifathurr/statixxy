...
class DlgProject(wx.Dialog, config_ui.ConfigUI):
    ...
    def __init__(self, parent, fil_proj=None, *, read_only=False):
        ...
        lbl_scroll_down = wx.StaticText(self.scroll_con_dets, -1,
            _('(scroll down for details of all your SQL database engines)'))
        ## default dbe
        ...
        for dbe_szr in getdata.get_dbe_szrs(dlg_proj=self,
                scroll=self.scroll_con_dets, lblfont=lblfont,
                read_only=self.read_only):
            self.szr_con_dets.Add(dbe_szr, 0, wx.GROW|wx.ALL, 10)
    ...
    def on_ok(self, _event):
        ...
        if self.read_only:
            ...
            try:
                dets = getdata.gui_connection_info(self, default_dbe)
                default_dbs, default_tbls, con_dets = dets
            except Exception:  ## any errors at individual level
                return
            ## overall errors
            enough_completed = proj_name and bool(con_dets)
            if not enough_completed:
                wx.MessageBox(
                    _('Not enough details completed to save a project file'))
                return
            default_dbe_lacks_con = default_dbe not in mg.DBE_MODULES.keys()
