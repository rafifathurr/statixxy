...
class SettingsEntry:
    ...
    def __init__(self, frame, panel, grid_size, col_dets,
        ...
        """
        :param tuple grid_size: dimensions of grid - note - if set too narrow
         won't be able to click into it until focus lost and restored
         (mysteriously!); or dialog resized; etc.
        ...
        ## grid control
        x, _y = grid_size
        col_types = [col_det['coltype'] for col_det in self.col_dets]
        has_text_browser = COL_TEXT_BROWSE in col_types
        if has_text_browser and x < mg.MIN_GRID_WIDTH_FOR_TEXT_BROWSER:
            raise Exception("grid width too small - triggers bug")
        ...
        for col_idx, col_det in enumerate(self.col_dets):
            ...
            for row_idx in range(self.rows_n):
                ## must make a separate editor and renderer for each cell
        ...
        self.colwidths = [None, ] * len(self.col_dets)  ## init
        for col_idx, col_det in enumerate(self.col_dets):
        ...
        if self.read_only:
            ...
            self.grid.SetGridCursor(self.current_row_idx, 0) # triggers OnSelect
            self.grid.MakeCellVisible(self.current_row_idx, 0)
        self.control = None
    ...
    def get_settings_data(self):
        """
        As non-mutating version of update_settings_data() so I can gradually
        unpick the confusing catastrophe which is the way I wired up settings
        collection from grids :-(
        """
