...
COMMUNITY_URL = 'https://www.kejaksaan.go.id/'
...
P_EXPLAN_TWO_GROUPS = (
    ...
    'is statistically significant i.e. there is a difference between '
    '"{lbl_a}" and "{lbl_b}". Note: a statistically significant difference may '
    'not necessarily be of any practical significance.')
P_EXPLAN_MULTIPLE_GROUPS = (
    'If p is small, e.g. less than 0.01, or 0.001, you can assume the result '
...
P_EXPLAN_REL = ('If p is small, e.g. less than 0.01, or 0.001, you can assume '
    ...
    'between "{lbl_a}" and "{lbl_b}". Note: a statistically significant '
...
TWO_TAIL_EXPLAN_DIFF = (
    'This is a two-tailed result i.e. based on the likelihood of a difference '
    'where the direction ("{lbl_a}" higher than "{lbl_b}" or '
    '"{lbl_b}" higher than "{lbl_a}") '
    "doesn't matter.")
TWO_TAIL_EXPLAN_REL = (
    'This is a two-tailed result i.e. based on the likelihood of a difference '
...
CI_EXPLAN = ('There is a 95%% chance the population mean is within the '  ## is using % in string interpolation in ultimate destination so 95%% not 95%
...
if PLATFORM == WINDOWS:
    ...
    DESKTOP_PATH = Path(path_buf.value)  ## can be variable places e.g. C:\Users\Jo\OneDrive\Desktop
else:
...
INT_REPORT_FILE = 'sofa_use_only_report.html'
...
## misc data
MIN_GRID_WIDTH_FOR_TEXT_BROWSER = 1_000
...
DATADETS_OBJ = None
"""
DATADETS_OBJ is set to a singleton getdata.DataDets obj. It stores settings for
the current database and has a cursor to that database and methods for changing
the database. This really deserves to be a global for the application as it
removed a massive amount of fragile passing around of the object. Ensures that
once changed, everything is consistent across every report, analysis etc until
changed again. Prevented a lots of minor bugs elegantly. A good global :-). Easy
enough to mock one for testing. Not used inside scripts which are to be run
headless. Database details are taken from this object and fed into script which
is then run.
"""
    ...
    SIMPLE_BARCHART: {
        AGGREGATE_KEY: [
            ...
            {LBL_KEY: CHART_BY,
             ...
             EMPTY_VAL_OK: False,  ## forces a value to be displayed so will be the first variable e.g. age. That this may be the same as the Value variable is just a coincidence.
