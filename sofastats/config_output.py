...
class DlgListVars(wx.Dialog):
    ...
    def on_lst_click(self, event):
        debug = False
        if self.just_opened:  ## seems to be triggered on open in Linux
            if mg.PLATFORM == mg.LINUX:
                if debug:
                    print('Just opened so no chance for a selection being made')
                event.Skip()
        ...
        event.Skip()
        for selection in self.lst_vars.GetSelections():
            self.lst_vars.Deselect(selection)
