...
def get_dbe_szr(dlg_proj, scroll, lblfont, *, read_only=False):
    ...
    ## default database
    dlg_proj.lbl_sqlite_default_db = wx.StaticText(scroll, -1,
        _('Default Database (name only):'))
    dlg_proj.lbl_sqlite_default_db.SetFont(lblfont)
    DEFAULT_DB = dlg_proj.sqlite_default_db if dlg_proj.sqlite_default_db else ''
    dlg_proj.txt_sqlite_default_db = wx.TextCtrl(
        scroll, -1, DEFAULT_DB, size=(250, -1))
    dlg_proj.txt_sqlite_default_db.Enable(not read_only)
    dlg_proj.txt_sqlite_default_db.SetToolTip(_('Default database (optional)'))
    ## default table
    dlg_proj.lbl_sqlite_default_tbl = wx.StaticText(scroll, -1,
        _('Default Table (optional):'))
    dlg_proj.lbl_sqlite_default_tbl.SetFont(lblfont)
    DEFAULT_TBL = dlg_proj.sqlite_default_tbl if dlg_proj.sqlite_default_tbl else ''
    dlg_proj.txt_sqlite_default_tbl = wx.TextCtrl(
        scroll, -1, DEFAULT_TBL, size=(250, -1))
    dlg_proj.txt_sqlite_default_tbl.Enable(not read_only)
    dlg_proj.txt_sqlite_default_tbl.SetToolTip(_('Default table (optional)'))
    szr_sqlite = wx.StaticBoxSizer(bx_sqlite, wx.VERTICAL)
    ## 3 SQLITE INNER
    szr_sqlite_inner = wx.BoxSizer(wx.HORIZONTAL)
    szr_sqlite_inner.Add(dlg_proj.lbl_sqlite_default_db, 0, wx.LEFT|wx.RIGHT, 5)
    szr_sqlite_inner.Add(dlg_proj.txt_sqlite_default_db, 0, wx.RIGHT, 10)
    szr_sqlite_inner.Add(dlg_proj.lbl_sqlite_default_tbl, 0, wx.LEFT|wx.RIGHT, 5)
    szr_sqlite_inner.Add(dlg_proj.txt_sqlite_default_tbl, 0, wx.RIGHT, 10)
    szr_sqlite.Add(szr_sqlite_inner, 0, wx.GROW)
    sqlite_col_dets = [{
        'col_label': DATABASE_FLD_LABEL, 
        'coltype': settings_grid.COL_TEXT_BROWSE,
        'colwidth': 400, 
        'file_phrase': _('Choose an SQLite database file')}]
    init_settings_data = dlg_proj.sqlite_data[:]  ## the data already entered we need to display
    init_settings_data.sort(key=lambda s: s[0])
    dlg_proj.sqlite_grid = settings_grid.SettingsEntry(frame=dlg_proj,
        panel=scroll, grid_size=(mg.MIN_GRID_WIDTH_FOR_TEXT_BROWSER, 100),
        col_dets=sqlite_col_dets,
        init_settings_data=init_settings_data, settings_data=[],
        read_only=read_only, force_focus=True)
    dlg_proj.sqlite_settings_data = dlg_proj.sqlite_grid.get_settings_data()
    """
    ...
    for row_idx, db_path in enumerate([x[0] for x in init_settings_data]):
        ...
        if db_is_default_sofa_db(db_name):
            dlg_proj.sqlite_grid.grid.SetRowAttr(row_idx, attr)
    szr_sqlite.Add(dlg_proj.sqlite_grid.grid, 1, wx.GROW|wx.ALL, 5)
    return szr_sqlite
...
def gui_connection_info(dlg_proj, *, mandatory):
    ...
    :param bool mandatory: not useful for SQLite - it always has the default
     SOFA db configured. Only included because following a standard interface
    ...
    if dlg_proj.sqlite_grid.new_is_dirty:
        msg = (_("The SQLite details on the new row have not been "
            "saved. Select the \"%s\" field in the new row and press Enter")
            % DATABASE_FLD_LABEL)
        wx.MessageBox(msg)
        dlg_proj.sqlite_grid.SetFocus()
        raise Exception(msg)
    dlg_proj.sqlite_settings_data = dlg_proj.sqlite_grid.get_settings_data()
    ## check and collect details
    lacks_default_sofa_db = True
    dbe_con_dets = {}
    if dlg_proj.sqlite_settings_data:
        db_names = []
        for db_path, in dlg_proj.sqlite_settings_data:  ## e.g. [("C:\.....\my_sqlite_db",), ...]
            ...
            dbe_con_dets[db_name_key] = new_sqlite_dic
        if lacks_default_sofa_db:
            msg = "The default SOFA database is missing"
            wx.MessageBox(msg)
            raise Exception(msg)
    default_db_val = dlg_proj.txt_sqlite_default_db.GetValue()
    default_tbl_val = dlg_proj.txt_sqlite_default_tbl.GetValue()
    has_sqlite_con = bool(dbe_con_dets)
    defaults_but_no_conns = (
        (default_db_val or default_tbl_val) and not has_sqlite_con)
    conns_but_no_default_sofa_db = has_sqlite_con and lacks_default_sofa_db
    if defaults_but_no_conns:
        msg = (_("The SQLite details are partially complete - either add a "
            "database or clear the SQLite default database and table"))
        wx.MessageBox(msg)
        dlg_proj.txt_sqlite_default_db.SetFocus()
        raise Exception(msg)
    elif conns_but_no_default_sofa_db:
        msg = (_("The sofa default database \"%s\"must be included in "
            "the project.") % mg.SOFA_DB)
        wx.MessageBox(msg)
        dlg_proj.txt_sqlite_default_db.SetFocus()
        raise Exception(msg)
