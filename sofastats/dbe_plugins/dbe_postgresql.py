...
def get_dbe_szr(dlg_proj, scroll, lblfont, *, read_only=False):
    ...
    ## default database
    dlg_proj.lbl_pgsql_default_db = wx.StaticText(
        scroll, -1, _('Default Database:'))
    dlg_proj.lbl_pgsql_default_db.SetFont(lblfont)
    pgsql_default_db = (
        dlg_proj.pgsql_default_db if dlg_proj.pgsql_default_db else '')
    dlg_proj.txt_pgsql_default_db = wx.TextCtrl(
        scroll, -1, pgsql_default_db, size=(250, -1))
    dlg_proj.txt_pgsql_default_db.Enable(not read_only)
    dlg_proj.txt_pgsql_default_db.SetToolTip(
        _('Default database (optional if user is postgres)'))
    ## default table
    dlg_proj.lbl_pgsql_default_tbl = wx.StaticText(
        scroll, -1, _('Default Table (schema.table):'))
    dlg_proj.lbl_pgsql_default_tbl.SetFont(lblfont)
    pgsql_default_tbl = (
        dlg_proj.pgsql_default_tbl if dlg_proj.pgsql_default_tbl else '')
    dlg_proj.txt_pgsql_default_tbl = wx.TextCtrl(
        scroll, -1, pgsql_default_tbl, size=(250, -1))
    dlg_proj.txt_pgsql_default_tbl.Enable(not read_only)
    dlg_proj.txt_pgsql_default_tbl.SetToolTip(
        _('Default table (optional). Schema optional if public'))
    ## host
    dlg_proj.lbl_pgsql_host = wx.StaticText(scroll, -1, _('Host:'))
    dlg_proj.lbl_pgsql_host.SetFont(lblfont)
    pgsql_host = dlg_proj.pgsql_host if dlg_proj.pgsql_host else ''
    dlg_proj.txt_pgsql_host = wx.TextCtrl(scroll, -1, pgsql_host, size=(100,-1))
    dlg_proj.txt_pgsql_host.Enable(not read_only)
    dlg_proj.txt_pgsql_host.SetToolTip(_('Host e.g. localhost, or remote:3307'))
    ## 5432 is the default port for PostgreSQL
    ## user
    dlg_proj.lbl_pgsql_user = wx.StaticText(scroll, -1, _('User:'))
    dlg_proj.lbl_pgsql_user.SetFont(lblfont)
    pgsql_user = dlg_proj.pgsql_user if dlg_proj.pgsql_user else ''
    dlg_proj.txt_pgsql_user = wx.TextCtrl(scroll, -1, pgsql_user, size=(100, -1))
    dlg_proj.txt_pgsql_user.Enable(not read_only)
    dlg_proj.txt_pgsql_user.SetToolTip(_('User e.g. postgres'))
    ## password
    dlg_proj.lbl_pgsql_pwd = wx.StaticText(scroll, -1, _('Password:'))
    dlg_proj.lbl_pgsql_pwd.SetFont(lblfont)
    pgsql_pwd = dlg_proj.pgsql_pwd if dlg_proj.pgsql_pwd else ''
    dlg_proj.txt_pgsql_pwd = wx.TextCtrl(
        scroll, -1, pgsql_pwd, size=(300, -1), style=wx.TE_PASSWORD)
    dlg_proj.txt_pgsql_pwd.Enable(not read_only)
    dlg_proj.txt_pgsql_pwd.SetToolTip(_('Password'))
    ## 2 pgsql
    szr_pgsql = wx.StaticBoxSizer(bx_pgsql, wx.VERTICAL)
    ## 3 pgsql INNER
    ## 4 pgsql INNER TOP
    szr_pgsql_inner_top = wx.BoxSizer(wx.HORIZONTAL)
    ## default database
    szr_pgsql_inner_top.Add(
        dlg_proj.lbl_pgsql_default_db, 0, wx.LEFT|wx.RIGHT, 5)
    szr_pgsql_inner_top.Add(
        dlg_proj.txt_pgsql_default_db, 0, wx.GROW|wx.RIGHT, 10)
    ## default table
    szr_pgsql_inner_top.Add(
        dlg_proj.lbl_pgsql_default_tbl, 0, wx.LEFT|wx.RIGHT, 5)
    szr_pgsql_inner_top.Add(
        dlg_proj.txt_pgsql_default_tbl, 0, wx.GROW|wx.RIGHT, 10)
    ## 4 pgsql INNER BOTTOM
    szr_pgsql_inner_btm = wx.BoxSizer(wx.HORIZONTAL)
    ## host
    szr_pgsql_inner_btm.Add(dlg_proj.lbl_pgsql_host, 0, wx.LEFT|wx.RIGHT, 5)
    szr_pgsql_inner_btm.Add(dlg_proj.txt_pgsql_host, 0, wx.RIGHT, 10)
    ## user
    szr_pgsql_inner_btm.Add(dlg_proj.lbl_pgsql_user, 0, wx.LEFT|wx.RIGHT, 5)
    szr_pgsql_inner_btm.Add(dlg_proj.txt_pgsql_user, 0, wx.RIGHT, 10)
    ## password
    szr_pgsql_inner_btm.Add(dlg_proj.lbl_pgsql_pwd, 0, wx.LEFT|wx.RIGHT, 5)
    szr_pgsql_inner_btm.Add(dlg_proj.txt_pgsql_pwd, 1, wx.GROW|wx.RIGHT, 10)
    #2 combine
    szr_pgsql.Add(szr_pgsql_inner_top, 0, wx.GROW|wx.ALL, 5)
    szr_pgsql.Add(szr_pgsql_inner_btm, 0, wx.ALL, 5)
    return szr_pgsql
...
def gui_connection_info(dlg_proj, *, mandatory):
    ...
    :rtype: tuple
    """
    default_db = dlg_proj.txt_pgsql_default_db.GetValue()
    pgsql_default_db = default_db if default_db else None
    default_tbl = dlg_proj.txt_pgsql_default_tbl.GetValue()
    lacks_schema = '.' not in default_tbl
    if default_tbl and lacks_schema:
        default_tbl = f'public.{default_tbl}'
    pgsql_default_tbl = default_tbl if default_tbl else None
    pgsql_host = dlg_proj.txt_pgsql_host.GetValue()
    pgsql_user = dlg_proj.txt_pgsql_user.GetValue()
    pgsql_pwd = dlg_proj.txt_pgsql_pwd.GetValue()
    ...
    incomplete_pgsql = (
        (dirty or mandatory) and (not has_pgsql_con or missing_db))
    if incomplete_pgsql:
        msg = _('The PostgreSQL details are incomplete')
        if mandatory:
            msg += f' yet {mg.DBE_PGSQL} is selected as the default database'
        if missing_db:
            msg += '.' + _(
                " A default database is required unless the user is 'postgres'")
        wx.MessageBox(msg)
        dlg_proj.txt_pgsql_default_db.SetFocus()
        raise Exception(msg)
    default_db = pgsql_default_db if pgsql_default_db else None    
    default_tbl = pgsql_default_tbl if pgsql_default_tbl else None
    if has_pgsql_con:
        dbe_con_dets = {'user': pgsql_user, 'password': pgsql_pwd}
        host_bits = pgsql_host.split(':')
        if 1 <= len(host_bits) <= 2:
            host = host_bits[0].strip()
            dbe_con_dets['host'] = host
            if len(host_bits) == 2:
                port = host_bits[1].strip()
                dbe_con_dets['port'] = port
        else:
