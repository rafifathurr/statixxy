...
def get_dbe_szr(dlg_proj, scroll, lblfont, *, read_only=False):
    ...
    ## default database
    dlg_proj.lbl_mysql_default_db = wx.StaticText(
        scroll, -1, _('Default Database:'))
    dlg_proj.lbl_mysql_default_db.SetFont(lblfont)
    mysql_default_db = (
        dlg_proj.mysql_default_db if dlg_proj.mysql_default_db else '')
    dlg_proj.txt_mysql_default_db = wx.TextCtrl(
        scroll, -1, mysql_default_db, size=(200, -1))
    dlg_proj.txt_mysql_default_db.Enable(not read_only)
    dlg_proj.txt_mysql_default_db.SetToolTip(_('Default database (optional)'))
    ## default table
    dlg_proj.lbl_mysql_default_tbl = wx.StaticText(
        scroll, -1, _('Default Table:'))
    dlg_proj.lbl_mysql_default_tbl.SetFont(lblfont)
    mysql_default_tbl = (
        dlg_proj.mysql_default_tbl if dlg_proj.mysql_default_tbl else '')
    dlg_proj.txt_mysql_default_tbl = wx.TextCtrl(
        scroll, -1, mysql_default_tbl, size=(200, -1))
    dlg_proj.txt_mysql_default_tbl.Enable(not read_only)
    dlg_proj.txt_mysql_default_tbl.SetToolTip(_('Default table (optional)'))
    ## host
    dlg_proj.lbl_mysql_host = wx.StaticText(scroll, -1, _('Host:'))
    dlg_proj.lbl_mysql_host.SetFont(lblfont)
    mysql_host = dlg_proj.mysql_host if dlg_proj.mysql_host else ''
    dlg_proj.txt_mysql_host = wx.TextCtrl(scroll, -1, mysql_host, size=(100, -1))
    dlg_proj.txt_mysql_host.Enable(not read_only)
    dlg_proj.txt_mysql_host.SetToolTip(_('Host e.g. localhost, or remote:3307'))
    ## user
    dlg_proj.lbl_mysql_user = wx.StaticText(scroll, -1, _('User:'))
    dlg_proj.lbl_mysql_user.SetFont(lblfont)
    mysql_user = dlg_proj.mysql_user if dlg_proj.mysql_user else ''
    dlg_proj.txt_mysql_user = wx.TextCtrl(scroll, -1, mysql_user, size=(100, -1))
    dlg_proj.txt_mysql_user.Enable(not read_only)
    dlg_proj.txt_mysql_user.SetToolTip(_('User e.g. root'))
    ## password
    dlg_proj.lbl_mysql_pwd = wx.StaticText(scroll, -1, _('Password:'))
    dlg_proj.lbl_mysql_pwd.SetFont(lblfont)
    mysql_pwd = dlg_proj.mysql_pwd if dlg_proj.mysql_pwd else ''
    dlg_proj.txt_mysql_pwd = wx.TextCtrl(
        scroll, -1, mysql_pwd, size=(300, -1), style=wx.TE_PASSWORD)
    dlg_proj.txt_mysql_pwd.Enable(not read_only)
    dlg_proj.txt_mysql_pwd.SetToolTip(_('Password'))
    ## 2 MYSQL
    szr_mysql = wx.StaticBoxSizer(bx_mysql, wx.VERTICAL)
    ## 3 MYSQL INNER
    ## 4 MYSQL INNER TOP
    szr_mysql_inner_top = wx.BoxSizer(wx.HORIZONTAL)
    ## default database
    szr_mysql_inner_top.Add(
        dlg_proj.lbl_mysql_default_db, 0, wx.LEFT|wx.RIGHT, 5)
    szr_mysql_inner_top.Add(
        dlg_proj.txt_mysql_default_db, 0, wx.GROW|wx.RIGHT, 10)
    ## default table
    szr_mysql_inner_top.Add(
        dlg_proj.lbl_mysql_default_tbl, 0, wx.LEFT|wx.RIGHT, 5)
    szr_mysql_inner_top.Add(
        dlg_proj.txt_mysql_default_tbl, 0, wx.GROW|wx.RIGHT, 10)
    ## 4 MYSQL INNER BOTTOM
    szr_mysql_inner_btm = wx.BoxSizer(wx.HORIZONTAL)
    ## host 
    szr_mysql_inner_btm.Add(dlg_proj.lbl_mysql_host, 0, wx.LEFT|wx.RIGHT, 5)
    szr_mysql_inner_btm.Add(dlg_proj.txt_mysql_host, 0, wx.RIGHT, 10)
    ## user
    szr_mysql_inner_btm.Add(dlg_proj.lbl_mysql_user, 0, wx.LEFT|wx.RIGHT, 5)
    szr_mysql_inner_btm.Add(dlg_proj.txt_mysql_user, 0, wx.RIGHT, 10)
    ## password
    szr_mysql_inner_btm.Add(dlg_proj.lbl_mysql_pwd, 0, wx.LEFT|wx.RIGHT, 5)
    szr_mysql_inner_btm.Add(dlg_proj.txt_mysql_pwd, 1, wx.GROW|wx.RIGHT, 10)
    ## 2 combine
    szr_mysql.Add(szr_mysql_inner_top, 0, wx.GROW|wx.ALL, 5)
    szr_mysql.Add(szr_mysql_inner_btm, 0, wx.ALL, 5)
    return szr_mysql
...
def gui_connection_info(dlg_proj, *, mandatory):
    ...
    if ':' in raw_host:
        ...
        mysql_host = raw_host
        mysql_port = 3306
    mysql_user = dlg_proj.txt_mysql_user.GetValue()
    mysql_pwd = dlg_proj.txt_mysql_pwd.GetValue()
    ...
    incomplete_mysql = (dirty or mandatory) and not has_mysql_con
    if incomplete_mysql:
        msg = _('The MySQL details are incomplete')
        if mandatory:
            msg += f' yet {mg.DBE_MYSQL} is selected as the default database'
        wx.MessageBox(msg)
        dlg_proj.txt_mysql_default_db.SetFocus()
        raise Exception(msg)
    if has_mysql_con:
        dbe_con_dets = {
        ...
        dbe_con_dets = {}
    return mysql_default_db, mysql_default_tbl, dbe_con_dets
