...
Ubuntu: Can't use wand to test for specific resource limits because the Ubuntu
packaged version is so stale it doesn't include wand.resource.limits :-(
"""
import os
import shutil
from subprocess import Popen, PIPE

from PIL import Image, ImageChops  ## actually the Pillows fork of PIL
import wx
...
def copy_output():
    ...
    for filname in sorted_names:
        if filname.endswith('.png'):
            imgname = str(mg.INT_COPY_IMGS_PATH / filname)
            do.AddFile(imgname)
...
class Pdf2Img:
    ...
    def _get_convert(platform):
        if platform == mg.WINDOWS:
            ...
            """
            Assumes the framework path is the first path in PATH, so we can just
            reference 'gs' in delegates.xml without a fully qualified file name,
            AND that the environment variable MAGICK_CONFIGURE_PATH has been set
            to the framework path so that imagemagick convert can find
            delegates.xml (and colors.xml) and thus make use of the largely
            self-contained gs (ghostscript) binary there.
    ...
    def pdf2img(pdf_path, img_pth_no_ext, bgcolour=mg.BODY_BACKGROUND_COLOUR,
        ...
        for i in range(n_pages):
            ...
            if err:
                err = str(err, encoding='utf-8').lower()
                if (mg.PLATFORM == mg.LINUX
                        and ('operation not allowed' in err
                            and 'security policy' in err)):
                    msg = (
                    "ImageMagick's policy.xml file needs editing to allow "
                    "ImageMagick to convert PDFs to images. In ubuntu/debian "
                    "sudo gedit /etc/ImageMagick-6/policy.xml and change "
                    '\n<policy domain="coder" rights="none" pattern="PDF" />'
                    '\nto'
                    '\n<policy domain="coder" rights="all" pattern="PDF" />'
                    )
                elif ('cache resources exhausted' in err
                        or 'image width exceeds user limit' in err):
                    msg = ('Unable to make these images at this '
                        f'resolution ({output_dpi} dpi) by converting from PDF '
                        'version. Please try again at a lower resolution.')
                    if mg.PLATFORM == mg.LINUX:
                        msg += ('\nAnother option to try is editing policy.xml '
                        'which sets limits on the resources ImageMagick is '
                        'allowed to use making images. In ubuntu/debian '
                    ...
                    msg = 'Unable to make these images successfully.'
                wx.MessageBox(msg + f'\n\nOrig error: {err}')
