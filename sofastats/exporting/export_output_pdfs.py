...
def get_raw_pdf(html_path, pdf_path, width='', height=''):
    ...
    try:
        ...
        try:
            ...
            pass
        rel_url_of_html = os.path.split(url)[1]
        path_of_html = os.path.split(html_path)[0]
        if mg.PLATFORM == mg.WINDOWS:  ## using Pyinstaller
            drive_of_html = os.path.splitdrive(html_path)[0]
            cmd_make_pdf = (
                f'{drive_of_html} && cd "{path_of_html}" && '
                f'"{export_output.EXE_TMP}\\wkhtmltopdf.exe" '
                f'{width} {height} "{rel_url_of_html}" "{pdf_path}"')
            print(cmd_make_pdf)
