...
There can be problems clicking on buttons outside of the data editor while in
the midst of editing cells. So it is crucial to check self.any_editor_shown and
self.dbtbl.new_is_dirty and avoid allowing changes to the table until the
editing is finished. Warn user if they are not going to see a change as
expected.
"""
...
def open_data_table(parent, var_labels, var_notes, var_types, val_dics, *,
    ...
    if not dd.has_unique:
        ...
        if rows_n > 200_000:  ## fast enough as long as column resizing is off
            msg = _("This table has %s rows. Do you wish to open it?") % rows_n
            retval = wx.MessageBox(
                msg, caption=_("LONG REPORT"), style=wx.YES_NO)
            if retval == wx.NO:
                return
        wx.BeginBusyCursor()
...
class TblEditor(wx.Dialog, config_ui.ConfigUI):
    ...
    def __init__(self, parent, var_labels, var_notes, var_types, val_dics, *,
        ...
        self.set_up_tbl()  ## needs to run before evaluating self.uses_default_max_dec_pts
        ...
        extra_top_cols = 2 if self.uses_default_max_dec_pts else 0
        szr_top = wx.FlexGridSizer(
            rows=1, cols=4 + extra_top_cols, hgap=5, vgap=5)
        szr_top.AddGrowableCol(2 + extra_top_cols, 2)  ## idx, propn
    ...
    def _disable_read_only_cols(self):
        """
        Disable read-only columns, set them to the read-only colour, and
        identify the first writable column index. This is where the cursor
        should land when moving from end of new row.
        """
    ...
    def set_up_tbl(self, *, initial_set_up=True):
        """
        :param bool initial_set_up: how we set up a table depends on whether we
         are setting up for the first time or simply refreshing it. If a
         editable table we will add a new row on end if initial set-up but not
         otherwise.
        """
        ...
        if self.read_only:
            self.col2select = 0
            self.grid.SetGridCursor(0, self.col2select)
            ...
            self.col2select = self._disable_read_only_cols()
            if initial_set_up:
                ## start at new line
                new_row_idx = self.dbtbl.GetNumberRows() - 1
                self.focus_on_new_row(new_row_idx, self.col2select)
                self.current_row_idx = new_row_idx
                self.set_new_row_ed(new_row_idx)
        self.current_col_idx = self.col2select
    ...
    def on_dp_spin(self, evt):
        ...
        if self.any_editor_shown or self.dbtbl.new_is_dirty:
            wx.MessageBox(_("Unable to change the number of decimal points "
                "displayed during unsaved edits or data additions"))
            evt.Skip()
            return
        self.set_up_tbl(initial_set_up=False)
    ...
    def set_new_row_ed(self, row_idx):
        """
        Set cell editor for cells in new row. Always a text editor so we can
        have dots as missing values and add data quickly in bulk.
        """
        for col_idx in range(len(self.dd.flds)):
            self.grid.SetCellEditor(
                row_idx, col_idx, wx.grid.GridCellTextEditor())
    ...
    def on_btn_filter(self, event):
        ...
        if retval != wx.ID_CANCEL:
            ...
            self.set_up_tbl()
            if debug: print('Finished set_up_tbl')
