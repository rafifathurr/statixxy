...
class DlgStatsSelect(wx.Dialog):
    ...
    def __init__(self, proj_name):
        ...
        if not self.tight_layout:
            ...
            self.form_height = 600 if in_win else 620
            self.diff_top = 125
        ...
        if mg.PLATFORM == mg.WINDOWS:
            self.config_left += 10
        self.btn_lift = 0 if in_win else 4
        ...
        ## close button
        close_y_pos = self.form_height- 42 if in_win else self.form_height -62
        self.btn_close = wx.Button(
            self.panel, wx.ID_CLOSE, pos=(self.form_width-100, close_y_pos))
    ...
    def on_paint(self, event):
        ...
        get_help_fs = lib.GuiLib.get_font_size_to_fit(text=get_help_txt,
            max_width=get_help_max_width, font_sz=get_help_font_sz,
            min_font_sz=11)
